...
await sql.query(`DELETE FROM price_snapshots WHERE source = $1`, [SOURCE]);

// Chunked multi-row INSERT — one round trip per 500 rows.
for (let i = 0; i < rows.length; i += 500) {
  ...
  await sql.query(
    `INSERT INTO price_snapshots (price_per_troy_oz, source, captured_at) VALUES ${values}`,
    params,
  );
}
