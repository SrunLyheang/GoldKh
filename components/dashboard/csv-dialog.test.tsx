...
describe("CsvDialog import preview", () => {
  ...
  it("enables commit once at least one row is valid", async () => {
    ...
    expect(screen.getByRole("button", { name: /Import 1 row/i })).toBeEnabled();
  });

  it("ignores stale file reads from older selections", async () => {
    const user = userEvent.setup();
    render(
      <CsvDialog
        open
    ...
    const delayedFirst = new Promise<string>((resolve) => {
    ...
    resolveFirst!(
      `${HEADER}\nbuy,2,chi,3000,USD,2026-08-01,\nbuy,3,chi,3000,USD,2026-08-01,\n`,
    );

    expect(
      await screen.findByRole("button", { name: /Import 1 row/i }),
    ).toBeEnabled();
    expect(
      screen.queryByRole("button", { name: /Import 2 rows/i }),
