...
export function EmptyState({ onAddClick }: { onAddClick: () => void }) {
  return (
    <div className="glass-surface flex flex-col items-center justify-center px-6 py-16 text-center">
      ...
      <p className="tt-heading tt-eyebrow mt-5 text-body text-foreground">{t.empty.title}</p>
      <p className="mt-1.5 max-w-xs text-detail text-muted-foreground">
        {t.empty.description}
      </p>
      <ol className="mt-6 flex max-w-sm flex-col gap-2.5 text-left">
        {t.empty.steps.map((step, i) => (
          <li key={i} className="flex gap-3 text-detail text-muted-foreground">
      ...
      <div className="mt-7">
        <Magnetic strength={10}>
          <Button onClick={onAddClick}>
            ...
            <span className="tt-label text-label">{t.transactions.addTransaction}</span>
          </Button>
        </Magnetic>
      </div>
    </div>
