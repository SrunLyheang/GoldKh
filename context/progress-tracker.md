...
- **2026-09-01:** Merged `feat/auth-split-card` into `main`. Three
  ...
  (its own `#0f0c08` gold-lit background + `SparkleField` + mascot
  brand panel) supersedes `main`'s `<AuthAuroraBackground>` and the
  standalone logo, so those were dropped from the pages; `main`'s
  `<BackToWelcome>` link is preserved, now rendered inside the
  `<AuthShell>` form panel below the Clerk form. `AuthAuroraBackground`
  + its test and the `.auth-aurora-*` rules in `globals.css` stay in
  the tree but are no longer referenced. Both dictionary key sets
  (`nav.signOut/signingOut/signOutError` and
  `settings.signOut/signOutDescription/signedInAs`) survived the
  auto-merge. tsc, eslint, 441 tests green.
